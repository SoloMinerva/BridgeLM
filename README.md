...
flowchart TB
    ...
    B4 --> B5["结构化自动评测<br/>2模型 × 40prompt × 5指标"]
    ...
    style B5 fill:#fdedec,stroke:#e74c3c,color:#2c3e50
```
...
| Adaptor 存储 | **1.0 MB**（vs 全参 360 MB，节省 **99.7%**） |
| KV Cache 加速 | CPU 平均 **3.27x** · 最大 **6.37x** |
| SFT 质量提升 | 评分 **+49%**（1.30 → 1.94，DeepSeek judge） |
| LoRA vs 全参（1k 步）| 生成质量达全参 **72%**（1.39 vs 1.94） |
...
| [02-自研 BridgeLM 主线](Readme/项目全景图/02-自研%20BridgeLM%20主线.md) | 数据处理 / Tokenizer / 模型设计 / Pretrain / SFT / LoRA / 能力边界 |
| [03-推理与系统能力增强](Readme/项目全景图/03-推理与系统能力增强.md) | 文本生成流程 / KV Cache 优化与 Benchmark / chat.py 多轮对话 |
| [04-Qwen 迁移与结构化输出](Readme/项目全景图/04-Qwen%20迁移与结构化输出主线.md) | 迁移动机 / InstructIE 数据 pipeline / Qwen LoRA 微调 / 导出 |
| [05-评测、验证与部署闭环](Readme/项目全景图/05-评测、验证与部署闭环.md) | 通用评测体系 / 结构化自动评测 / Alias 归一化 |
