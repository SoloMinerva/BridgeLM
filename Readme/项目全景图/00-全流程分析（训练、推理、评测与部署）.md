...
---
# BridgeLM 全流程分析（训练、推理、评测与部署）

> 这篇文档的目标不是重复“项目总览”里的成果表，而是像 `cs336/推理&训练.md` 那样，从**数据流和执行路径**的角度，把 BridgeLM 项目真正跑起来时的完整链路讲清楚：输入是什么，经过哪些模块，产出什么，为什么这样设计，以及两条主线之间如何衔接。
...
前面的 [[01-项目总览]]、[[02-自研 BridgeLM 主线]]、[[03-推理与系统能力增强]]、[[04-Qwen 迁移与结构化输出主线]] 更强调“按主题讲故事”。这篇文档换一种组织方式：
...
BridgeLM 不是单线项目，而是两条并行的能力验证线：

- **主线 A+B：自研 BridgeLM 链路**
...
- 它们在评测阶段汇合：同一套结构化评测脚本同时比较 `qwen_base / qwen_lora / BridgeLM_sft / BridgeLM_lora`
...
- 它定义了模型的“输入语言”
- BridgeLM 的 embedding 层、lm_head、tokenized memmap 都依赖这一词表
...
- BridgeLM 证明了完整训练链路可行
- 但在结构化评测中，`BridgeLM_sft / BridgeLM_lora` 的 JSON Parse% = 0%
...
%%{init: {"theme": "base", "themeVariables": {"background": "#ffffff", "primaryColor": "#f0fdf4", "primaryBorderColor": "#22c55e", "primaryTextColor": "#0f172a", "lineColor": "#64748b"}}}%%
flowchart TB
    B1["4 个模型<br>qwen_base / qwen_lora / BridgeLM_sft / BridgeLM_lora"] --> B2["40 条结构化 prompts"]
    B2 --> B3["统一自动检测"]
...
BridgeLM 项目最终验证的不是单一技术点，而是一套端到端方法论：
...
flowchart TB
    ...
    class C1,C2,C3,C4,C5,C6 chain;
```

每一步都有对应产物、对应指标、对应脚本、对应验证方法。项目不是停在”loss 降了”这一层，而是走到了”行为可测、结果可复现、模型可导出”这一层。
