...
class SFTNpyDataset(Dataset):
    """Pre-tokenized SFT dataset stored as (N, max_len) int16 npy arrays."""

    def __init__(self, input_ids_path: str | Path, labels_path: str | Path) -> None:
        super().__init__()
        import numpy as np
        self._input_ids = np.load(input_ids_path, mmap_mode="r")
        self._labels = np.load(labels_path, mmap_mode="r")
        if self._input_ids.shape != self._labels.shape:
            raise ValueError("input_ids and labels must have same shape")

    def __len__(self) -> int:
        return self._input_ids.shape[0]

    def __getitem__(self, index: int):
        ids = torch.from_numpy(self._input_ids[index].astype("int64"))
        labs = torch.from_numpy(self._labels[index].astype("int64"))
        return ids, labs
