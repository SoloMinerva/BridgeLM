...
priority: P0
file: BridgeLM/model/lora.py (173行)
---
...
**state_dict 只存 A/B 的意义：** 全参 360 MB vs LoRA adaptor 仅 **1.0 MB**，节省 **99.7%** 存储。可以快速切换不同任务的 adaptor 而不需要保存完整模型。
...
| LoRA adaptor 多大？ | BridgeLM 上仅 1MB（全参 377MB 的 0.27%）|
