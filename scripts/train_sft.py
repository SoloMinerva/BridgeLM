...
from bridgelm.model.lora import (
    apply_lora_to_model,
    get_lora_params,
    get_lora_state_dict,
...
from bridgelm.training import AdamW
from bridgelm.training import SFTDataset, SFTNpyDataset
from bridgelm.training import load_model_state, load_checkpoint, masked_cross_entropy, save_checkpoint
from bridgelm.training import learning_rate_schedule as get_lr_cosine_schedule
...
def build_parser(defaults: dict[str, object]) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train bridgelm on SFT conversations.")
    ...
    parser.add_argument("--min-lr", type=float, default=defaults.get("min_lr", 1e-6))
    parser.add_argument("--warmup-steps", type=int, default=defaults.get("warmup_steps", 0))
    ...
    parser.add_argument("--grad-accum", type=int, default=defaults.get("grad_accum", 1))
...
def evaluate(model: TransformerLM, loader: DataLoader, device: str, max_batches: int = 50) -> float:
    model.eval()
    total_loss = 0.0
    total_weight = 0.0
...
def main() -> None:
    ...
    if train_path.is_dir():
        train_ds = SFTNpyDataset(
            train_path / "train_input_ids.npy",
            train_path / "train_labels.npy",
        )
        valid_ds = SFTNpyDataset(
            train_path / "valid_input_ids.npy",
            train_path / "valid_labels.npy",
        )
        print(f"Loaded pre-tokenized SFT: train={len(train_ds)}, valid={len(valid_ds)}")
    ...
    print(f"  Effective batch: {eff_batch}, warmup={args.warmup_steps}, min_lr={args.min_lr}")
    ...
    for step in pbar:
        ...
        lr = get_lr_cosine_schedule(completed_step, args.lr, args.min_lr,
