...
Usage:
    ...
    python scripts/run_instructie_eval.py --skip-bridgelm   # Qwen models only
    python scripts/run_instructie_eval.py --skip-qwen      # bridgelm models only
"""
...
# ══════════════════════════════════════════════════════════════════════
# bridgelm Inference Backend
# ══════════════════════════════════════════════════════════════════════

def load_bridgelm_model(
    ...
    """Load a bridgelm model (optionally with LoRA). Returns (model, tokenizer, eos_token_id)."""
    import torch.nn as nn
    from bridgelm.model import TransformerLM
    from bridgelm.model.lora import load_lora_state_dict, merge_lora
    ...
    if lora_adaptor_path is not None and lora_adaptor_path.exists():
        from bridgelm.model.lora import apply_lora_to_model
...
def run_evaluation(
    ...
    bridgelm_sft_path: str = "./outputs/sft_baseline/ckpt_final.pt",
    bridgelm_lora_path: str = "./outputs/sft_lora/ckpt_final.pt",
    bridgelm_lora_adaptor: str = "./outputs/sft_lora/lora_adaptor.pt",
    ...
    if not skip_bridgelm:
        models_to_eval.append(("bridgelm_sft", "bridgelm", None))
        models_to_eval.append(("bridgelm_lora", "bridgelm", bridgelm_lora_adaptor))
    ...
    for model_name, backend, adaptor in models_to_eval:
        ...
        if backend == "qwen":
            ...
            model, tokenizer, eos_id = load_bridgelm_model(
                Path(bridgelm_sft_path if model_name == "bridgelm_sft" else bridgelm_lora_path),
                device=device,
                dtype=torch.float32,
...
def parse_args():
    ...
    parser.add_argument("--skip-bridgelm", action="store_true", help="Skip bridgelm models")
    ...
    parser.add_argument("--bridgelm-sft-path", type=str, default="./outputs/sft_baseline/ckpt_final.pt")
    parser.add_argument("--bridgelm-lora-path", type=str, default="./outputs/sft_lora/ckpt_final.pt")
    parser.add_argument("--bridgelm-lora-adaptor", type=str, default="./outputs/sft_lora/lora_adaptor.pt")
...
if __name__ == "__main__":
    ...
    run_evaluation(
        ...
        device=args.device,
        skip_bridgelm=args.skip_bridgelm,
        skip_qwen=args.skip_qwen,
        qwen_base_path=args.qwen_base_path,
        qwen_adaptor_path=args.qwen_adaptor_path,
        bridgelm_sft_path=args.bridgelm_sft_path,
        bridgelm_lora_path=args.bridgelm_lora_path,
        bridgelm_lora_adaptor=args.bridgelm_lora_adaptor,
